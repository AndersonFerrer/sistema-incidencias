...
**Scope**: Backend (nuevo módulo `dashboard/` + 1 endpoint `GET /api/dashboard`) + Frontend (reemplazar los mocks de `pages/dashboard/data.ts` por un fetch real contra el nuevo endpoint).

This capability spec is the synced baseline for the `dashboard` capability after **one** archived change (`dashboard-real`); the archive history below records what the change contributed and which drifts were applied during archive.

> **Archive history** (baseline evolution; each archive added/extended requirements on top of the prior baseline):
>
> - **`dashboard-real`** (2026-07-14, archived): seeded the baseline. **Backend primary** — new `dashboard/` module with `GET /api/dashboard?rango={7d|30d|90d|all}` endpoint, 7 parametric SQL aggregations (KPIs, byCategoria, tendenciaSemanal, recientes, tiempoPromedioResolucion), per-role scope injection at DAO WHERE, 5 DTOs, mapper. **Frontend** — new `dashboard-service.ts`, props-driven components, range selector, loading skeleton, mock arrays stripped from `data.ts`. **Postman** — new "Dashboard" folder with 5 sample responses (3 role-scoped + 400 + 401). 32 Given/When/Then scenarios covered. Verdict PASS. See `openspec/changes/archive/dashboard-real/archive-report.md`.
